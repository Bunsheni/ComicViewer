...
namespace ComicViewer
{
    ...
    {        
        ...
        public CtWebService(IWebConnection web)
        {
            webConnection = web;
        }
        ...
        public async Task<List<CtComic>> getPageComics(CtModelList old_comics, int page)
        {
            int count;
            List<CtComic> new_comics = new List<CtComic>();
            string url, tempstr, infostr;
            int errorcount = 0;
            string Language = languageStringList[(int)CtLanguage.KOREAN];
            string path = "https://hitomi.la/index-" + Language + "-";
            ...
            try
            {
                ...
                {
                    Console.WriteLine($"{page}페이지다운로드 실패");
                    return new_comics;
                }
                if (infostr.Contains("<div class=\"gallery-content\">"))
                {
                    ...
                    while (infostr.Contains("<a href=\"/galleries/"))
                    {
                        ...
                        {
                            Library.extractionString(infostr, out infostr, "href=\"/galleries/", ".html", out tempstr);
                            if (old_comics.GetModel(tempstr) == null)
                            {
                                CtComic comic = await searchWorkInfoFromWeb(tempstr);
                                new_comics.Add(comic);
                                infostr = infostr.Substring(infostr.IndexOf("</div>\n</div>"));
                                count++;
                            }
                        }
