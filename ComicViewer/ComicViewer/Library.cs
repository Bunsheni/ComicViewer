...
namespace ComicViewer
{
    public static class StringExtensions
    {
        ...
        public static string RemoveMark(this string str)
        {
            return str.RemoveChar('「', '」', '『', '』', '!', ',', '(', ')', '[', ']', '<', '>', '.', '"', '\'', '?', '~', '-', '+', '*', '&', '%', ':', ';', '《', '》');
        }

        public static string RemoveChar(this string str, params char[] chs)
        {
            string res = str;
    ...
    public static class Library
    {
        ...
        {
            if ((str.IndexOf(word) == 0 || (str.IndexOf(word) > 0 && str[str.IndexOf(word) - 1] == ' ')))
                if (str.IndexOf(word) + word.Length == str.Length || (str.IndexOf(word) + word.Length < str.Length && str[str.IndexOf(word) + word.Length] == ' '))
                    return true;
            return false;
